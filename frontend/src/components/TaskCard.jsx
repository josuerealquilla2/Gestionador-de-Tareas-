...
export default function TaskCard({ task, onToggle, onEdit, onDelete }) {
  ...
  const borderColor = task.is_completed
    ? 'border-teal-600/40'
    : 'border-blue-600/40';
  const dotColor = task.is_completed ? 'bg-teal-400' : 'bg-blue-400';
      ...
      {editing ? (
        <div>
          <input
            ...
            autoFocus
            className="w-full bg-slate-900 text-white border border-slate-600 rounded-lg px-3 py-2 mb-2 focus:outline-none focus:border-blue-500 transition-colors"
          />
          <textarea
            value={descripcion}
            onChange={e => setDescripcion(e.target.value)}
            rows={2}
            className="w-full bg-slate-900 text-white border border-slate-600 rounded-lg px-3 py-2 mb-3 resize-none focus:outline-none focus:border-blue-500 transition-colors"
          />
          <div className="flex gap-2">
            <button
              onClick={handleSave}
              className="flex-1 bg-blue-700 text-white text-sm font-semibold py-2 rounded-lg hover:bg-blue-600 transition-colors"
            >
              Guardar
            </button>
            <button
          ...
          <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-700">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="text-slate-400 hover:text-blue-400 transition-colors text-sm px-2 py-1 rounded hover:bg-slate-700"
              >
                Editar
              </button>
              {confirmDelete ? (
